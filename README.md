...
- `k8s_platform`: one of `k3s`, `eks`, `aks`, `gke`, `rke2`, `dkp`, `ocp`
...
- `REACT_INSTALL`: whether React event-driven automation is deployed
- `tmp_dir`: where install artifacts are staged on the control machine

React is configured separately. Its `REACT_*` variables, covering the hostname, namespace, admin account, and image versions, live in [`playbooks/group_vars/all.yml`](./playbooks/group_vars/all.yml) rather than in `default.config.yml`.
...
- **7 platform installers**: k3s, EKS, AKS, GKE, RKE2, DKP, OCP
- **10 roles**: cluster setup, Ascender, Ledger, React, backup, restore, migration
- **5 lifecycle playbooks**: backup, restore, migrate, upgrade_postgres, create_bundle
- **React**: optional event-driven automation, built on the Ansible EDA operator
...
- Report bugs and platform requests via [GitHub Issues](https://github.com/ctrliq/ascender-k8s-install/issues).
